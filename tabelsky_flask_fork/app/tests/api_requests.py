...
def base_request(
    http_method: Literal["get", "post", "delete", "patch"], path: "str", *args, **kwargs
) -> dict:
...
def patch_user(
    user_id: int, email: str, password: str = None, token: str = None
) -> dict:
    params = {
        key: value
        for key, value in (("email", email), ("password", password))
        if value is not None
    }
    return base_request(
        "patch", f"users/{user_id}", json=params, headers={"token": token}
    )
