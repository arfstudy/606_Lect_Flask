...
import pytest
from tests.api_requests import (ApiError, delete_user, get_user, login,
                                patch_user, register)
...
def test_register_simple_password():
    ...
    assert error.value.message == {
        "status": "error",
        "description": [
            {"loc": ["password"], "msg": "password is to easy", "type": "value_error"}
        ],
    }
...
def test_register_existed(new_user):
    ...
    assert error.value.message == {
        "status": "error",
        "description": f"such user already exists",
    }
